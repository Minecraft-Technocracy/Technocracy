...
class RenderEventListener {
    private val underwaterGrayscale =
            ResourceLocation("technocracy.foundation", "textures/extra/underwater_grayscale.png")
    ...
    fun handleFluidBlockOverlayEvent(event: RenderBlockOverlayEvent) {
        ...
        if (event.overlayType == RenderBlockOverlayEvent.OverlayType.WATER) {
            ...
            if (block is BlockFluidBase) {
                ...
                val brightness = 0.8
                val color = block.getFogColor(player.world, pos, state, player, Vec3d(1.0, 1.0, 1.0), 0.0f)
                        .scale(brightness)
