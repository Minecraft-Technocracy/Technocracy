...
import net.cydhra.technocracy.foundation.blocks.general.centrifugeBlock
...
class CentrifugeRecipeCategory(guiHelper: IGuiHelper) : AbstractRecipeCategory<CentrifugeRecipeCategory.CentrifugeRecipeWrapper>(
        guiHelper,
        centrifugeBlock,
        RecipeManager.RecipeType.CENTRIFUGE,
        CentrifugeRecipeWrapper::class.java,
        TCCategoryUid.CENTRIFUGE
) {

    private val progressbarDrawable: IDrawable = DefaultProgressBar(21, 38, Orientation.RIGHT, null, null).getDrawable(100, guiHelper)

    override fun setRecipe(layout: IRecipeLayout, wrapper: CentrifugeRecipeWrapper, ingredients: IIngredients) {
        val stacks = layout.itemStacks
        stacks.init(0, true, 0, 36)
        stacks.init(1, false, 44, 36)
        stacks.init(2, false, 67, 36)
